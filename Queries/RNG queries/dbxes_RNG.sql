...
BEGIN
	...
	WHERE
		(SELECT CAST(value AS DATETIME2(3)) FROM attribute WHERE [key] = 'time:timestamp' AND id IN (SELECT attr_id FROM event_has_attribute WHERE event_id = eha.event_id)) >= @interval_start
			AND
		(SELECT CAST(value AS DATETIME2(3)) FROM attribute WHERE [key] = 'time:timestamp' AND id IN (SELECT attr_id FROM event_has_attribute WHERE event_id = eha.event_id)) < @interval_end
	GROUP BY a.[type] , a.[key] , a.value
