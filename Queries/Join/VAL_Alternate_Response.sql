...
FROM (
	SELECT	a.trace_id,
			...
			CASE WHEN EXISTS (
					SELECT * FROM @event c 
					WHERE c.task = a.task AND a.log_id = c.log_id AND a.trace_id = c.trace_id 
						AND c.[time] > a.[time] AND c.[time] < b.[time]
						AND NOT EXISTS (
							SELECT * FROM @event d
							WHERE d.task = b.task AND d.log_id = b.log_id AND d.trace_id = b.trace_id 
								AND d.[time] > a.[time] AND d.[time] < c.[time]
						)
					) 
				THEN NULL 
				ELSE b.[time]
