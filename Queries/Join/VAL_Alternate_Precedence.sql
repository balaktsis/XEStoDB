...
FROM (
	SELECT	a.trace_id,
			...
			CASE WHEN EXISTS (
					SELECT * FROM @event c 
					WHERE c.task = b.task AND b.log_id = c.log_id AND b.trace_id = c.trace_id 
						AND c.[time] > a.[time] AND c.[time] < b.[time]
						AND NOT EXISTS (
							SELECT * FROM @event d
							WHERE d.task = a.task AND d.log_id = a.log_id AND d.trace_id = a.trace_id 
								AND d.[time] > c.[time] AND d.[time] < b.[time]
						)
					)
				THEN NULL
				ELSE a.[time]
